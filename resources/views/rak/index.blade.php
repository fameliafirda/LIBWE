...
<div class="container mb-5">
    ...
    <div class="rak-container" id="rakContainer">
        @foreach($raks as $rak)
            <div class="rak-card {{ $rakId == $rak->id ? 'selected' : '' }}" 
                 data-rak-id="{{ $rak->id }}"
                 data-rak-judul="{{ $rak->judul }}"
                 data-rak-nomor="{{ $rak->nomor }}"
                 data-rak-deskripsi="{{ $rak->deskripsi }}">
                ...
                <div class="px-3 pb-3">
                    @foreach($rak->categories->take(4) as $cat)
                        <span class="category-tag">{{ $cat->nama }}</span>
                    @endforeach
                    @if($rak->categories->count() > 4)
                        <span class="category-tag">+{{ $rak->categories->count() - 4 }}</span>
                    @endif
                </div>
            </div>
        @endforeach
    ...
    <div class="search-filter-section" id="searchFilterSection">
        ...
        <div class="filter-label mb-2"><i class="fas fa-tags me-2"></i>Filter Kategori</div>
...
<script>
    ...
    $(document).on('click', '.rak-card', function() {
        ...
        // Load categories for this rak
        loadCategories(currentRakId);
    ...
    // Load categories untuk rak yang dipilih
    function loadCategories(rakId) {
        $.ajax({
            url: '{{ route("rak.getBooks") }}'.replace('/books', '/' + rakId + '/books'),
            method: 'GET',
            success: function(response) {
                if (response.rak && response.rak.categories) {
                    categories = response.rak.categories;
                }
                renderFilterButtons();
            },
            error: function() {
                // Fallback: ambil dari data rak card
                categories = [];
                renderFilterButtons();
            }
        });
    ...
    function renderFilterButtons() {
        // Ambil kategori unik dari buku yang akan ditampilkan
        ...
        if (currentRakId) {
            $.ajax({
                url: '{{ route("rak.books") }}',
                method: 'GET',
                data: { rak_id: currentRakId },
                async: false,
                success: function(response) {
                    if (response.success && response.books) {
                        response.books.forEach(book => {
                            if (book.category_nama) {
                                uniqueCategories.add(book.category_nama);
                            }
                        });
                    }
                }
        ...
        let buttonsHtml = '<button class="filter-chip active" data-category="all">📚 Semua Buku</button>';
        uniqueCategories.forEach(cat => {
            buttonsHtml += `<button class="filter-chip" data-category="${cat}">📖 ${cat}</button>`;
        });
    ...
    // Load books dengan AJAX
    function loadBooks() {
        if (!currentRakId) return;
        
        $('#booksSection').html('<div class="loading"><i class="fas fa-spinner fa-spin"></i><p class="mt-2">Memuat buku...</p></div>');
    ...
    function displayBooks(books, rak) {
        if (!books || books.length === 0) {
            let message = '';
            if (currentSearch) {
                message = `Tidak ditemukan buku dengan kata kunci "${currentSearch}"`;
            } else if (currentCategory !== 'all') {
                message = `Tidak ada buku dengan kategori "${currentCategory}"`;
            } else {
                message = 'Belum ada buku di rak ini';
            }
            ...
            $('#booksSection').html(`
                <div class="empty-state">
                    ...
                    <p>${message}</p>
                    ${(currentSearch || currentCategory !== 'all') ? '<button onclick="resetFilters()" class="btn btn-primary mt-3">Reset Filter</button>' : ''}
                </div>
            `);
            return;
        }
        ...
        let html = `
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4 style="color: var(--text-dark);">
                    <i class="fas fa-book-open me-2" style="color: var(--pastel-purple);"></i>
                    Buku di <span style="color: var(--pastel-blue);">${escapeHtml(rak?.judul || currentRakJudul || 'Rak')}</span>
                </h4>
                <span class="badge" style="background: linear-gradient(135deg, var(--pastel-pink), var(--pastel-purple)); padding: 8px 16px;">
                    ${books.length} Buku
                </span>
            </div>
        ...
        books.forEach(book => {
            const coverHtml = book.gambar 
                ? `<img src="/storage/${book.gambar}" class="book-cover" style="object-fit: cover; width: 100%; height: 180px;" alt="${escapeHtml(book.judul)}">`
            ...
            html += `
                <div class="book-card">
                    ...
                    <div class="book-info">
                        ...
                        <p class="book-author"><i class="fas fa-user me-1"></i>${escapeHtml(book.penulis)}</p>
                        <span class="book-category">${escapeHtml(book.category_nama)}</span>
                        <div class="mt-2 text-muted" style="font-size: 11px;">
                            <i class="fas fa-map-marker-alt me-1"></i> Rak ${escapeHtml(book.rak_nomor)}
                        </div>
                    </div>
                </div>
            `;
