...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="mb-1 fw-bold" style="color: #2d3748;">
                <i class="fas fa-plus-circle me-2" style="color: #8b5cf6;"></i>Catat Peminjaman Baru
            </h3>
    ...
    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show shadow-sm" role="alert" style="border-radius: 15px; border-left: 5px solid #38a169;">
    ...
    <form action="{{ route('pinjamans.store') }}" method="POST" id="pinjamanForm">
            ...
            <div class="card-body p-4">
                <div class="row g-4">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="nama" id="nama" class="form-control form-control-lg custom-input" value="{{ old('nama') }}" placeholder="Ketik nama lengkap..." required>
                    </div>
                    <div class="col-md-4">
                        <label for="kelas" class="form-label fw-semibold text-muted">Kelas <span class="text-danger">*</span></label>
                        <input type="text" name="kelas" id="kelas" class="form-control form-control-lg custom-input" value="{{ old('kelas') }}" placeholder="Contoh: 1A, 2B..." required>
                    </div>
                    <div class="col-md-4">
                        <label for="jenis_kelamin" class="form-label fw-semibold text-muted">Jenis Kelamin <span class="text-danger">*</span></label>
                        <select name="jenis_kelamin" id="jenis_kelamin" class="form-select form-select-lg custom-input" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" name="tanggal_pinjam" id="tanggal_pinjam" class="form-control form-control-lg custom-input" value="{{ old('tanggal_pinjam', date('Y-m-d')) }}" required>
                    </div>
                    <div class="col-md-4">
                        <label for="tanggal_kembali" class="form-label fw-semibold text-muted">Tanggal Kembali (Estimasi) <span class="text-danger">*</span></label>
                        <input type="date" name="tanggal_kembali" id="tanggal_kembali" class="form-control form-control-lg custom-input" value="{{ old('tanggal_kembali', date('Y-m-d', strtotime('+7 days'))) }}" required>
                    </div>
                    <div class="col-md-4">
                        <label for="status" class="form-label fw-semibold text-muted">Status Awal <span class="text-danger">*</span></label>
                        <select name="status" id="status" class="form-select form-select-lg custom-input bg-light" required>
        ...
        <div class="card border-0 shadow-sm mb-4" style="border-radius: 20px; overflow: hidden;">
            <div class="card-header bg-white border-bottom-0 pt-4 pb-2 px-4 d-flex justify-content-between align-items-center">
                ...
                <button type="button" class="btn btn-success shadow-sm" id="btnTambahBuku" style="border-radius: 50px; padding: 8px 20px; font-weight: 600; background-color: #10b981; border: none;">
            ...
            <div class="card-body p-4">
                ...
                <div id="buku-container">
                    <div class="book-row p-3 mb-3" style="background-color: #f8fafc; border-radius: 15px; border: 1px solid #e2e8f0;">
                        <label class="form-label fw-bold" style="color: #2d3748;">Judul Buku <span class="book-number text-success">1</span></label>
                        <div class="input-group input-group-lg">
                            ...
                            <button type="button" class="btn btn-primary px-4 btn-cek-buku fw-bold" style="border-radius: 0 10px 10px 0; background-color: #8b5cf6; border-color: #8b5cf6;">
        ...
        <div class="text-end mb-5">
            <button type="submit" class="btn btn-primary btn-lg px-5 shadow" id="submitBtn" style="border-radius: 50px; font-weight: 700; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none;">
...
<style>
    /* Styling khusus agar UI terlihat lebih soft & modern */
    body { 
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateBookNumbers() {
            ...
            rows.forEach((row, index) => {
                ...
                // Tampilkan tombol hapus jika baris buku lebih dari 1
                if (rows.length > 1) {
        ...
        btnTambah.addEventListener('click', function() {
            ...
            newRow.style = 'background-color: #f8fafc; border-radius: 15px; border: 1px solid #e2e8f0;';
            newRow.innerHTML = `
                <label class="form-label fw-bold" style="color: #2d3748;">Judul Buku <span class="book-number text-success">${rowCount}</span></label>
                <div class="input-group input-group-lg">
                    <span class="input-group-text bg-white border-end-0 text-muted"><i class="fas fa-book"></i></span>
                    <input type="text" name="judul_buku[]" class="form-control border-start-0 ps-0 custom-input-group judul-buku-input" placeholder="Ketik atau pilih judul buku dari daftar..." required autocomplete="off" list="daftarBuku">
                    ...
                    <button type="button" class="btn btn-danger px-3 btn-hapus-buku ms-2" style="border-radius: 10px;" title="Hapus baris ini">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
                <div class="status-buku mt-2 fw-medium ms-1"></div>
        ...
        // Event Listener Tombol Hapus & Cek (Event Delegation)
            ...
            if (e.target.closest('.btn-cek-buku')) {
                ...
                if (!judul) {
                    Swal.fire({ 
                        icon: 'warning', 
                        title: 'Field Kosong', 
                        text: 'Silakan ketik judul buku terlebih dahulu!',
        ...
        // Event Focusout (Kehilangan Fokus) untuk cek otomatis saat selesai mengetik/memilih
        ...
        // Fungsi AJAX Cek Ketersediaan Buku ke Server
        async function cekKetersediaanBuku(judul, statusEl, saranEl) {
            statusEl.innerHTML = '<span class="text-primary"><i class="fas fa-spinner fa-spin me-1"></i> Sedang memeriksa database...</span>';
            saranEl.innerHTML = '';
            
            try {
                // Menggunakan fungsi route() bawaan Laravel
                ...
                if (data.exists) {
                    statusEl.innerHTML = `<span class="text-success"><i class="fas fa-check-circle me-1"></i> Buku ditemukan! Stok tersisa: <strong>${data.stok}</strong> buku</span>`;
                    return true;
                } else {
                    statusEl.innerHTML = `<span class="text-danger"><i class="fas fa-times-circle me-1"></i> ${data.message}</span>`;
                    
                    // Berikan saran buku jika judul typo
